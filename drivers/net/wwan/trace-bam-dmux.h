...
/*
 * Tracepoints for the Qualcomm BAM-DMUX WWAN driver.
 *
 * Covers the scenarios that the downstream bam_dmux.c logged via IPC
 * logging (BAM_DMUX_LOG / DMUX_LOG_KERR), transposed into the upstream
 * architecture:
 *
 *   bam_dmux_pc          ← downstream: bam_dmux_smsm_cb (reconnect/disconnect)
 *   bam_dmux_power       ← downstream: bam_dmux_smsm_cb + disconnect_to_bam /
 *                                       reconnect_to_bam (SPS connect/disconnect)
 *   bam_dmux_resume_step ← downstream: ul_wakeup (waiting prev ack / wakeup ack /
 *                                       completion / complete)
 *   bam_dmux_channel     ← downstream: handle_bam_mux_cmd_open/close,
 *                                       msm_bam_dmux_open/close
 *   bam_dmux_rx          ← downstream: handle_bam_mux_cmd (magic/signal/cmd/ch/len)
 */
